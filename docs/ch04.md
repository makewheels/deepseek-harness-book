...
> **阅读提示**
> 本章保留“公司总部”的类比，但不会停在类比上。每个关键结论后面都给出配置、源码或运行证据。
...
Cordis 是 dsh 底下的插件框架。可以继续用公司的类比理解它，但每个类比都要对应一个真实机制（表4-2）。
...
假设一个插件需要执行工具。它声明依赖 `tools` 服务，然后通过 `ctx.tools` 调用，而不是直接导入某个工具注册表实现。

如果 `tools` 尚未出现，依赖它的插件会保持 `PENDING`，等服务就绪后再启动。也就是说，配置文件里的上下顺序不负责表达依赖，`inject` 才负责。

这就是可替换性的第一半：**消费者依赖稳定的服务名和接口，而不是具体提供方。** 只要新提供方履行同一接口，消费者不需要知道柜台后面换了人。

### Event：不认识对方，也能合作

服务适合直接调用；事件适合观察、拦截或扩展流程。例如工具执行前后的策略，不必修改 agent loop，可以监听 `tools/*` 事件。

Cordis 支持普通广播、并行/串行分发和 waterfall（瀑布式事件）。Waterfall 像一条可包裹的中间件链：监听器调用 `next()` 才会继续交给下游；不调用就会短路。审批、重写和策略插件因此可以加入流程，而不需要彼此直接导入。

官方的 [Cordis 入门](https://github.com/deepseek-ai/deepseek-harness/blob/99f6f02fecdb7dff40c3fbc9470f5907c29f74ca/docs/cordis-primer.zh.md)把这套关系概括为五个概念：插件、上下文、依赖注入、类型化事件和可逆 effect。

## 真正的关键不是“能插”，而是“能拔干净”

很多系统都能在运行时注册一个回调，难点是卸载后不留下幽灵资源：重复监听器、没有关闭的定时器、仍占端口的服务器、继续响应请求的旧实现。

Cordis 把插件产生的外部影响记为 effect。effect 在建立资源时返回一个 disposer；插件卸载时，Cordis 会执行这些 disposer。`ctx.on()`、`ctx.plugin()` 和服务注册等内置操作已经属于 effect，插件只需为框架不知道的资源——例如定时器、连接和文件 watcher——显式提供清理函数。
...
Fiber 不是只有“存在/不存在”两种状态。它会经历 `PENDING → LOADING → ACTIVE → UNLOADING → DISPOSED`；加载失败则进入 `FAILED`。

这让两种常见故障可以被区分：缺少所需服务时，插件仍在等待；`apply` 或配置校验抛错时，插件已经失败。完整 dsh 启动还会审计没有解析或没有激活的配置项并明确失败，而不是带着半棵插件树假装启动成功。

所以“可逆”不只是为了炫技式热更新。它首先是一条资源所有权规则：**谁在加载时建立影响，谁就必须在卸载时撤销影响。**
...
实际启动和 `--dump-config` 共用同一套组合算法。[`prepareProfile()`](https://github.com/deepseek-ai/deepseek-harness/blob/99f6f02fecdb7dff40c3fbc9470f5907c29f74ca/apps/cli/src/profile-boot.ts#L132-L170)按这个次序把各层交给 `composeEntries()`，因此打印出的树不是另一套近似配置，而是对启动组合的离线呈现。
...
官方 [`web-cordis` 示例](https://github.com/deepseek-ai/deepseek-harness/blob/99f6f02fecdb7dff40c3fbc9470f5907c29f74ca/examples/web-cordis/README.zh.md)允许 agent 检查当前 Cordis 进程，并在内存中挂载或卸载模型编写的临时插件。
