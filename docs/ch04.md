...
> **阅读提示**
> 本章保留"公司总部"和"换零件"的类比，但不会停在类比上。每个关键结论后面都给出配置、源码或运行证据。证据代码都放在折叠块里（默认收起）——不读代码的读者直接跳过读结论即可，不影响理解。
...
| `waterfall` | 沿监听链传递，每层可包装或短路 | 有 | 审批、拦截、改写 |
| `parallel` | 所有监听者并发执行并等待完成 | 无 | 并行收集副作用 |
| `serial` | 按序执行并等待，第一个有效回答胜出 | 有 | 决策前的最后关卡 |

严格说框架还有第五种 `bail`（拿到第一个有效回答即终止）；本书与官方入门手册一致，只列上面四种常见的。
...
下面是本章实际运行过的最小探针。把它保存到操作系统临时目录，然后在 DeepSeek Harness 仓库根目录用 `pnpm exec tsx <文件路径>` 运行（实验 4-2 要用；不读代码的读者可跳过折叠块直接读结论）：

??? note "证据｜最小生命周期探针与实际输出"
...
| 词 | 是什么 |
|---|---|
| profile | 一套具名组装：列出 bundle，保存自身的 patch，并可安装树外插件；`web` 和 `headless`（模板现有五个，见第 12 章）是随发行版交付的 |
...
实际启动和 `--dump-config` 共用同一套组合算法。[`composeProfile()`](https://github.com/deepseek-ai/deepseek-harness/blob/99f6f02fecdb7dff40c3fbc9470f5907c29f74ca/apps/cli/src/profile-boot.ts#L132-L170)按上述次序把各层交给 `composeEntries()`，因此打印出的树不是另一套近似配置，而是对启动组合的离线呈现。
...
| 动作 | 干什么 |
|---|---|
| `cordis_inspect_list` | 列出当前插件树里可查验的对象（提供方与注册项） |
| `cordis_inspect_query` | 按指定方法细查某个注册项 |
| `cordis_inspect_self` | 照镜子：查看当前挂着的插件和它们的包 |
...
| Profile 与配置导出测试 | **本次实际跑通** | `profile.spec.ts` 与 `config-dump.spec.ts` 共 20 个测试通过（基线时的数字；上游现已增至 45 个） |
