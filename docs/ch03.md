...
`examples/` 里的每台示例都能直接跑，官方给它们的定位是"展示主要接口和扩展点的可运行演示"。先认个脸：

**表3-3 六台官方样机车**

| 示例 | 演示什么 |
|---|---|
| `headless-agent` | 非交互式 agent：接一个任务，干完，按机器或人类可读格式输出 |
| `jsonrpc-agent` | 用 Python SDK + JSON-RPC 驱动的无人值守编码 agent |
| `acp-agent` | 面向程序化客户端的 ACP 自动化服务器，支持会话、权限和取消 |
| `mcp-memory` | 通过通用 MCP 客户端接第三方记忆服务器的可选 overlay |
| `web-schedule` | 会话内的持久提醒：`dsh web --patch ...` 一行挂上 |
| `web-cordis` | 能检查并修改内存中插件树的"自指" agent——dsh 自己改自己 |

第 11 章写你的第一个插件时，会回来借其中一台当模板。

## 五十个零件组，按器官归堆

`packages/` 里 50 个组，名字都像缩写，第一次看会懵。别慌——按第 1 章的五器官归堆，立刻清楚（表3-4）：
