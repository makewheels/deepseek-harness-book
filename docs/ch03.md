...
| `packages/` | 全部能力零件 | 51 个组，按"组 → 包"两级分文件夹 |
| `docs/` | 官方说明书 | 中英双语，仅顶层专题文档就有 19 篇，外加 cordis 教程和 cookbook |
| `examples/`（已退役） | 曾经的官方样机车 | 本书写作后上游整体退役（[退役提交](https://github.com/deepseek-ai/deepseek-harness/commit/4125514a08)）：现存示例移到 `packages/examples/` 与 `docs/cookbook/`，下节细说 |
...
| `website/` | dsh 官网的源码 |
| `.agents/` | 留给 AI agent 的设计笔记和技能清单，后记重点讲 |
| `patches/` | 第三方依赖补丁（目前只有 node-pty 的一份） |
| `snapshots/` | 录制会话的回放快照，给测试当输入和预期输出 |
| `AGENTS.md` | 写给 AI agent 的仓库工作守则——`CLAUDE.md` 只是它的快捷方式 |

## 样机车搬家了

本书初稿写的是顶层 `examples/` 下的六台官方样机车。写作之后上游做了结构调整，把这一区整体退役——现在仓库里已经没有顶层 `examples/` 了。示例搬去了两个地方（表3-3）：

**表3-3 示例的新家**

| 去处 | 里面有什么 |
|---|---|
| [`packages/examples/`](https://github.com/deepseek-ai/deepseek-harness/tree/master/packages/examples) | 供测试与自定义部署复用的组合 bundle，现存一台 `agent-spine-demo` 演示 |
| [`docs/cookbook/`](https://github.com/deepseek-ai/deepseek-harness/tree/master/docs/cookbook) | 9 篇实战菜谱（都有中文版）：加一个工具、加一个模型适配器、加远程 API、加设置卡片、写扩展等 |

想看一台完整组合长什么样，去翻 `agent-spine-demo`；想学怎么往车上加零件，直接读对应菜谱。第 11 章写你的第一个插件时走官方教程，需要整台组合当参照时再回来找它。

## 五十一个零件组，按器官归堆

`packages/` 里 51 个组，名字都像缩写，第一次看会懵。别慌——按第 1 章的五器官归堆，立刻清楚（表3-4）：
...
| 🚪 门面 | `api/` `sdk/` `acp/` `host/` `client/` `hooks/` `webhook/` | 对外接口、进程外 SDK、自动化协议、网页两半、与外部工具互通、外部事件入站 |
| 🧱 车架 | `boot/` `bundle/` `preset/` `extensions/` `settings/` `workspace/` `identity/` `typert/` | 启动、配方、预置、运行时自修改、设置、工作区、匿名身份 |
| 🧪 支持 | `examples/` `test-support/` `util/` `experimental/` `runtime-diagnostics/` | 演示组合、测试基建、底层工具、未发布原型、运行时不变量检查 |
...
- 组内还分"包"。比如 `llm/` 组里有 `llm`（抽象层）、`llm-deepseek`（DeepSeek 适配器）等七个包。**抽象和具体实现分开打包**，这是"换大脑不难"的工程真相。
...
想搞懂 dsh 某个设计"为什么长这样"，除了读代码，还可以去翻对应的设计笔记——这是第一手的决策记录。后记会专门讲从这套做法里能偷师什么。
...
| 第11章 | 第一个插件 | 官方插件教程 + `packages/examples/` |
| 第12章 | 组装自己的 agent | `boot/` `bundle/` `preset/` |
| 第13章 | 会话日志与续接 | `session/` `session-query/` |
| 第14章 | 编排：Workflow 与 Ralph | `workflow/` `subagent/` `goal/` |
| 第15章 | 规则自进化实战 | `extensions/` `hooks/` `skill/` `settings/` |
...
- **五大区**——apps、packages、docs、vendor，外加被上游退役的 examples（示例搬进 `packages/examples/` 和 `docs/cookbook/`）；另有 python、scripts、patches、snapshots、.agents 等小区
- **零件按器官归堆**——接大脑/记忆循环/手/规矩/目标/外设/门面/车架/支持，共 51 组
